...
class WizardHrPaySlip(models.TransientModel):
    ...
    file_data = fields.Binary('Archivo Generado',default=lambda self: self.env['wizard.hr.payslip'].search([])[-1].report)
    file_name = fields.Char('Nombre de archivo',default=lambda self: self.env['wizard.hr.payslip'].search([])[-1].report)
