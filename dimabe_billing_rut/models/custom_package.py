...
class CustomPackage(models.Model):
    ...
    canning_type = fields.Char(string="Tipo de Envase Interno")
    ...
    #    for item in self:
    #        canning_types = self.env['product.atribute'].mapped('value_ids').search([('name','=','Tipo de envase')])
    #        cannings = []
    #        for canning in canning_types:
