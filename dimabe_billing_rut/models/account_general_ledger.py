...
import logging
# from odoo.addons.account_reports.models.account_report import AccountReport as report
...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        lines = []
        aml_lines = []
        options_list = self.env['account.report'].sudo()._get_options_periods_list(options)
        unfold_all = options.get('unfold_all') or (self._context.get('print_mode') and not options['unfolded_lines'])
        date_from = fields.Date.from_string(options['date']['date_from'])
        company_currency = self.env['account.report'].sudo().env.company.currency_id
