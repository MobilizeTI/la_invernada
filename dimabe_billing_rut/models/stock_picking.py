...
class StockPicking(models.Model):
    ...
    def _compute_required_loading_week(self):
        for item in self:
            if item.required_loading_date:
                try:
                    year, week, day_of_week = item.required_loading_date.isocalendar()
                    item.required_loading_week = week
                except:
                    raise UserWarning('no se pudo establecer la semana de carga')
            else:
                item.required_loading_week = None
