    ...
    def generate_book(self):
        for item in self:
            ...
            for wk in array_worksheet:
                ...
                invoice = self.env['account.invoice'].search([('company_id.id','=',company.id),('type','=','in_invoice'),('state','=','paid')])
                row = 12
                for inv in invoice:
                    ...
                    sheet.write('F{}'.format(str(row)),inv.partner_id.display,merge_format_string)
                    row += 1
            workbook.close()
