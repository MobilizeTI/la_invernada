...
class AccountInvoiceLine(models.Model):
    ...
    @api.depends('price_unit', 'discount', 'discount_value', 'invoice_line_tax_ids', 'quantity',
    ...
    def _compute_price(self):
        for line in self:
            ...
            if line.discount_value:
                price_unit = currency.round(price_unit * quantity) - currency.round(line._get_discount_total())
                discount = 0.0
                quantity = 1
            if line.invoice_line_tax_ids:
                ...
                taxes = line.invoice_line_tax_ids.compute_all(price_unit, currency, quantity, product=line.product_id, partner=line.invoice_id.partner_id, discount=discount, uom_id=line.uom_id)
            if taxes:
                ...
                total_discount = int(decimal.Decimal(currency.round(line._get_discount_total())).to_integral_value())
                total -= total_discount
                line.price_subtotal = price_subtotal_signed = int(decimal.Decimal(total).to_integral_value())
            price_total = price_total_signed = taxes['total_included'] if (taxes and taxes['total_included']) else total
            if line.invoice_id.currency_id and line.invoice_id.currency_id != line.invoice_id.company_id.currency_id:
                ...
                price_total_signed = currency._convert(price_total_signed, line.invoice_id.company_id.currency_id, line.company_id or self.env.user.company_id, date or fields.Date.today())
            ...
            line.price_total_signed = price_total_signed * sign
...
class AccountInvoice(models.Model):
        ...
        for r in self:
            ...
            if r.type in ['in_invoice', 'in_refund']:
                if r.type == 'in_invoice':
                    # considerar los documentos marcados como facturas de compra
    ...
    def _compute_amount(self):
        ...
        self.amount_tax_company_signed = amount_tax_signed * sign

    @api.multi
    def _prepare_tax_line_vals_cl(self, account, account_analytic, analytic_tags, tax):
        ...
        vals = {
            ...
            'account_analytic_id': tax['analytic'] and account_analytic.id or False,
            'analytic_tag_ids': tax['analytic'] and analytic_tags.ids or False,
            'account_id': self.type in ('out_invoice', 'in_invoice') and (tax['account_id'] or account.id) or (tax['refund_account_id'] or account.id),
            'amount_retencion': tax['retencion'],
            'retencion_account_id': self.type in ('out_invoice', 'in_invoice') and (tax['refund_account_id'] or account.id) or (tax['account_id'] or account.id),
        }
        ...
        if not vals.get('account_analytic_id') and account_analytic and vals['account_id'] == account.id:
            vals['account_analytic_id'] = account_analytic.id
    ...
    def _get_key_tax_grouped(self, line):
        ...
        product = False
        tax_key = (line.account_id, line.account_analytic_id, line.analytic_tag_ids, product, line.invoice_line_tax_ids)
        return tax_key
        ...
        *********************************************************
        ...
        for line in self.invoice_line_ids:
            ...
            if (line.invoice_line_tax_ids and line.invoice_line_tax_ids[0].price_include) :# se asume todos losproductos vienen con precio incluido o no ( no hay mixes)
                if included or not tax_data: # genero error en caso de contenido mixto, en caso primer impusto no incluido segundo impuesto incluido
                    for t in line.invoice_line_tax_ids:
                        if t not in totales:
                            totales[t] = 0
            ...
            tax_data[tax_key]['subtotal'] += (line.price_unit * line.quantity)
            tax_data[tax_key]['quantity_sum'] += line.quantity
        return tax_data
    ...
    def get_taxes_values(self):
        ...
        for (account, account_analytic, analytic_tags, product, tax_recs), total_data in tax_data.items():
            ...
            for tax in values['taxes']:
                val = self._prepare_tax_line_vals_cl(account, account_analytic, analytic_tags, tax)
    ...
    def name_get(self):
        ...
        result = []
        for inv in self:
            invoice_name = f"{inv.number or TYPES[inv.type]} {inv.name or ''}"
            if inv.type in ('out_invoice', 'out_refund') and inv.sii_document_number and inv.document_class_id:
                invoice_name = f"{inv.document_class_id.report_name or inv.document_class_id.name} {inv.sii_document_number}{inv.name or ''}"
            if inv.type in ('in_invoice', 'in_refund') and inv.reference and inv.document_class_id:
                invoice_name = f"{inv.document_class_id.report_name or inv.document_class_id.name} {inv.reference}{inv.name or ''}"
            result.append((inv.id, invoice_name))
        return result

    @api.model
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        args = args or []
        res = super(AccountInvoice, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
        if not res:
            recs = self.search([('sii_document_number', operator, name)] + args, limit=limit)
            if not recs:
    ...
    def _refreshRecords(self):
        ...
        for line in self.invoice_line_ids:
            if not line.account_id:
                continue
            # esta funcion pretende tomar los impuestos de la company actual
            # para evitar un error en multicompany cuando ya se ha ingresado las lineas
            # y luego se cambia la company en la cabecera, se quedaban los impuestos de la otra company
            # pero trae problema cuando se crea la factura desde una OC y en la linea de la OC se asigna un impuesto diferente al de la ficha del producto
            # esta funcion vuelve a tomar los impuestos de la ficha del producto e ignora los de la OC
            # asi que para mantener compatibilidad en ambos escenarios
            # solo cambiar impuestos si pertenecen a otra company, pero si no hay impuestos de otra company, mantener los impuestos originales
            if not line.invoice_line_tax_ids.filtered(lambda x: x.company_id.id != self.company_id.id):
                continue
            tax_ids = []
...
a VAT."))
    ...
    @api.one
    @api.constrains('sii_document_number', 'partner_id', 'company_id', 'type',
                    'document_class_id')
    ...
    @api.onchange('sii_document_number', 'document_class_id')
    def set_reference(self):
        if self.type in ['in_invoice', 'in_refund'] and self.sii_document_number and self.document_class_id:
            self.reference = "%s %s" %(
    ...
    def action_cancel(self):
        for invoice in self:
            if invoice.document_class_id and invoice.sii_xml_request:
                if invoice.sii_xml_request.state in ('Enviado', 'Aceptado', 'Reparo', 'Proceso', 'Anulado'):
                    raise UserError("No se permite anular el documento, ya ha sido enviado y aceptado por el SII")
    ...
    def action_move_create(self):
        for obj_inv in self:
            ...
            # si ya tiene numero de folio, escribir ese para pasarse al asiento contable
            elif obj_inv.journal_document_class_id and obj_inv.sii_document_number:
        ...
        for obj_inv in self:
            invtype = obj_inv.type
            if invtype in ('in_invoice',  'in_refund') and  obj_inv.reference and obj_inv.reference.isnumeric() and not obj_inv.sii_document_number:
    ...
    def _receptor(self):
        ...
        if not commercial_partner_id.rut() and not self._es_boleta() and not self._nc_boleta():
    ...
    def _validaciones_caf(self, caf):
        if not self.partner_id.document_number and not self._es_boleta() and not self._nc_boleta():
...
#                lines['RUTMandante']
            ...
            if not no_product:
                ...
                if currency_id:
                    ...
                    lines['OtrMnda']['PrcOtrMon'] = round(currency_base._convert(
                        line.price_unit, currency_id, self.company_id, self.date_invoice, round=False), 6)
    ...
    def _dte(self, n_atencion=None, is_for_libro=False):
        dte = {}
        ...
        if self.referencias and not (is_for_libro and self.document_class_id.sii_code in (33, )):
    ...
    def get_dte_claim(self):
        ...
        try:
            ...
            if e.args[0][0] == 503:
                raise UserError('%s: Conexión al SII caída/rechazada o el SII está temporalmente fuera de línea, reintente la acción' % (str(e)))
