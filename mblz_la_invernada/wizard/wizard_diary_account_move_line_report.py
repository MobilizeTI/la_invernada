...
class WizardDiaryAccountMoveLine(models.TransientModel):
    ...
    def generate_diary_book(self):
        ...
        for item in self:
            ...
            for wk in array_worksheet:
                ...
                titles = ['Fecha', 'Diario', 'Cuenta', 'Cód.Cuenta', 'Analítica', 'Ref', 'Empresa', 'Débito', 'Crédito',
                          'Divisa', 'Match']
                ...
                # Totales
                sheet.write(row + 2, col + 6, 'Total General', formats['title'])
                sheet.write(row + 2, col + 7, data_invoice['total_general_debit'], formats['total'])
                sheet.write(row + 2, col + 8, data_invoice['total_general_credit'], formats['total'])

        workbook.close()
        with open(file_name, "rb") as file:
            file_base64 = base64.b64encode(file.read())
        file_name = 'Libro Diario {} Desde {} Hasta {}.xlsx'.format(self.company_get_id[0].name,
    ...
    def set_data_for_excel(self, sheet, row, invoices, titles, formats):
        total_general_debit, total_general_credit = 0.0, 0.0
        for inv in invoices:
            col = 0
            ...
            row = data['row']
            col = data['col']
            total_general_debit += data['total_debit']
            total_general_credit += data['total_credit']
            row += 1
    ...
    def set_data_invoice(self, sheet, col, row, inv, invoices, titles, formats):
        ...
        line_out = {'sheet': sheet, 'row': row, 'col': col, 'total_debit': total_debit, 'total_credit': total_credit}
