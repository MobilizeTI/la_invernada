            ...
            for res in result:
                if res.partner_id.id:
                    ...
                    data.append({
                        ...
                        'VarietyName': res.move_ids_without_package.filtered(lambda a: 'envase' not in str.lower(a.product_id.categ_id.name)).product_id.get_variety(),
                        'LotNumber': res.name,
        ...
        if sale_order:
            ...
            for mes in mesagge:
                if mes.tracking_value_ids.filtered(lambda a: a.new_value_char == 'Realizado'):
                    stock_picking = request.env['stock.picking'].search([('id','=',mes.res_id)])
                    picking_data.append({
                        'Container' : stock_picking.container_number,
                        'Date':mes.date
                    })
