...
class AccountInvoice(models.Model):
    ...
    def _get_number_lines(self):
        number_lines = 30 # este numero se deberia personalizar segun cada pais, o empresa
        return number_lines
    
    def _get_invoice_line_key_to_group(self, invoice_line):
        line_key = (invoice_line.id, )
        if invoice_line.product_id:
            line_key = (invoice_line.product_id.id, )
        return line_key
    
    @api.multi
    def _get_invoice_lines_grouped(self):
        invoice_line_data = OrderedDict()
        AIL = self.env['account.invoice.line']
        for invoice_line in self.invoice_line_ids.sorted('product_id'):
            line_key = self._get_invoice_line_key_to_group(invoice_line)
            invoice_line_data.setdefault(line_key, AIL.browse())
            invoice_line_data[line_key] |= invoice_line
        return list(invoice_line_data.values())
    
    @api.multi
    def _get_count_invoice_lines(self):
        return len(self._get_invoice_lines_grouped())
    ...
    def split_invoice_document(self, invoice_id):
        ...
        if invoice.type in ('out_invoice', 'out_refund'):
            ...
            if document_number > 1:
                ...
                invoice_lines_data = invoice._get_invoice_lines_grouped()
                invoice_lines = self.env['account.invoice.line'].browse()
                for invoice_line_tmp in invoice_lines_data[number_lines:]:
                    invoice_lines |= invoice_line_tmp
    ...
    def action_invoice_open(self):
        if not self.env.context.get('skip_tax_validation', False):
            messages = []
            for invoice in self:
                messages = []
                for line in invoice.invoice_line_ids.filtered(lambda x: not x.invoice_line_tax_ids and not x.display_type):
                    messages.append("Producto: %s" % (line.name))
                if messages:
                    raise UserError("Hay Lineas de factura que no tienen impuestos, por favor verifique y asigne el impuesto correcto.\n %s" % 
                                    ("\n".join(messages)))
