...
class CustomerIndex(models.Model):
    ...
    def get_latest_version(self, old_version):
        if old_version:
            self.env.cr.execute('select id from customer_index WHERE id >= %s order by id desc limit 1' % (old_version,))
        else:
...
class CustomerLog(models.Model):
    ...
    @api.model
    def _get_fields_to_sync_pos(self):
        return ['name', 'ref', 'vat', 'street', 'city', 'phone', 'email', 'customer', 'property_product_pricelist', 'property_account_position_id']

    @api.multi
    def write(self, values):
        if self and set(self._get_fields_to_sync_pos()).intersection(set(values.keys())):
