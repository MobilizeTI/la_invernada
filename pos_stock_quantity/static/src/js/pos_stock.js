...
odoo.define('pos_stock_quantity.pos_stock', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function (session, attributes) {
            ...
            var self = this;
            this.chrome.call('bus_service', 'onNotification', this, function (notifications) {
        ...
        on_stock_notification: function (stock_quant) {
            ...
            var is_for_my_location = true;
            var quants_my_location = stock_quant.filter(function (item) {
                return item.location_id[0] === self.config.stock_location_id[0];
            });
            if (self.config.location_only && quants_my_location.length <= 0){
            	is_for_my_location = false;
            }

            if (this.config && this.config.show_qty_available && product_ids.length > 0 && is_for_my_location) {
        ...
        is_allow_out_of_stock: function(){
        	return this.config.allow_out_of_stock;
        },
    ...
    models.Order = models.Order.extend({
    	add_product: function(product, options) {
    		var self = this;
    		if (this.pos.config.show_qty_available && product.type === 'product' && !this.pos.is_allow_out_of_stock()) {
                var qty_available = this.pos.db.qty_by_product_id[product.id] || 0;
                if (qty_available <= 0 || qty_available  < this.pos.config.limit_qty) {
                	self.pos.gui.show_popup('error',{
    		        	'title': "Disponibilidad de Stock",
    		        	'body': _.str.sprintf("El producto %s no tiene suficiente stock, no se puede agregar", [product.display_name]),
    		        });
                    return;
                }
            }
    ...
    models.Orderline = models.Orderline.extend({
        set_quantity: function (quantity, keep_price) {
            ...
            if (!this.pos.config.show_qty_available
                || this.pos.is_allow_out_of_stock()
        ...
        check_reminder: function () {
            var self = this;
            var qty_available = this.pos.db.qty_by_product_id[this.product.id] || 0;
