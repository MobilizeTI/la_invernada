        ...
        if self.picking_type_code == 'outgoing':
            ...
            for move_line in self.move_line_ids_without_package:
                if self.picking_type_id.warehouse_id.id == 17 and self.picking_type_code != 'outgoing':
                    ...
                    raise models.UserError(float_is_zero(move_line.qty_done, precision_digits=precision_digits) for move_line in self.move_line_ids.filtered(lambda m: m.state not in ('done', 'cancel')))
                    return super(StockPicking, self).button_validate()
