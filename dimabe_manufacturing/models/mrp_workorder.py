    ...
    def _compute_potential_lot_planned_ids(self):
        for item in self:
            if item.potential_serial_planned_ids.mapped(
                ...
                item.potential_serial_planned_ids = self.env['stock.production.lot.serial'].search([('consumed_to_production_id.id','=',item.production_id),('stock_production_lot_id.product_id','=',product_id.id)])
