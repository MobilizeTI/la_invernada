...
class MrpWorkorder(models.Model):
    ...
    def process_serial(self, serial_number):
        raise models.ValidationError(f'{self.sudo().read()}')
        ...
        if serial.product_id not in workorder.material_product_ids:
        ...
        workorder.component_id = serial.product_id
        serial.write({
            'reserved_to_production_id': workorder.production_id.id,
            'used_in_workorder_id': workorder.id,
            'consumed': True
        })
        serial.stock_production_lot_id.update_stock_quant(workorder.production_id.location_src_id.id)
        serial.stock_production_lot_id.update_kg(serial.stock_production_lot_id.id)
        line_new = self.env['stock.move.line']
        if serial.producer_id in workorder.producers_id:
            workorder.write({
                'producers_id': [(4, serial.producer_id.id)]
            })
        move = workorder.production_id.move_raw_ids.filtered(lambda a: a.product_id.id == serial.product_id.id)
        if move.active_move_line_ids:
            ...
            if not line.lot_produced_id:
                line.write({
                    'lot_produced_id': workorder.final_lot_id.id
                })
            line.write({
                'qty_done': sum(workorder.potential_serial_planned_ids.filtered(
            ...
            line_new = self.env['stock.move.line'].create({
                ...
                'location_dest_id': workorder.env['stock.location'].search([('usage', '=', 'production')]).id,
        ...
        if workorder.active_move_line_ids.filtered(lambda a: not a.lot_id and a.product_id.id == serial.product_id.id):
            line_wo = workorder.active_move_line_ids.filtered(
            ...
            if line_wo:
                ...
                self.env['stock.move.line'].create({
                    ...
                    'location_dest_id': workorder.env['stock.location'].search([('usage', '=', 'production')]).id,
                    'location_id': workorder.production_id.location_src_id.id,
                    'move_id': workorder.move_raw_ids.filtered(lambda a: a.product_id.id == serial.product_id.id).id,
                    'product_uom_id': serial.product_id.uom_id.id,
                    'date': date.today(),
                    'qty_done': sum(workorder.potential_serial_planned_ids.filtered(
                        lambda a: a.stock_production_lot_id.id == serial.stock_production_lot_id.id).mapped(
        ...
        check = workorder.check_ids.filtered(
        ...
        check.write({
            ...
            'qty_done': sum(
                workorder.potential_serial_planned_ids.filtered(lambda a: a.product_id.id == serial.product_id.id).mapped(
                    'display_weight'))
        })
